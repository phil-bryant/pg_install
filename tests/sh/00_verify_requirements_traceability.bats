...
@test "Fails when header-bundled tags are used near file top" {
  ...
  cp "$(repo_root)/00_verify_requirements_traceability.sh" "${fixture_root}/00_verify_requirements_traceability.sh"
  run bash "${fixture_root}/00_verify_requirements_traceability.sh" "${fixture_root}/requirements/fixture-requirements.md" "${fixture_root}/fixture.sh"
  [ "$status" -ne 0 ]
...
@test "Requirements-only mode skips source/test traceability checks" {
  local fixture_root original_pwd
  fixture_root="$(mktemp -d)"
...
EOF
  cp "$(repo_root)/00_verify_requirements_traceability.sh" "${fixture_root}/verify_requirements_traceability.sh"
  original_pwd="$(pwd)"
  cd "${fixture_root}"
  run bash "./verify_requirements_traceability.sh"
  cd "${original_pwd}"
  [ "$status" -eq 0 ]
...
@test "Fails when discovered Go package tests do not include requirement tags" {
  ...
  original_pwd="$(pwd)"
  cd "${fixture_root}"
  run bash "./verify_requirements_traceability.sh" "./requirements/pkg/example-requirements.md" "./pkg/example.go"
  cd "${original_pwd}"
  [ "$status" -ne 0 ]
